...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Sayara Backend account.<br>
        ...
        &mdash; Sayara Backend Team
      `
      return sendMail({ toEmail: email, subject: 'Sayara Backend - Password Reset', content })
    })
